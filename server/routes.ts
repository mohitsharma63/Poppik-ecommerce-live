...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/login", async (req, res) => {
    const { email, password, city, state, pincode } = req.body;
    ...
    try {
      ...
      // Update user with city, state, pincode if provided
      ...
      const token = jwt.sign({ userId: foundUser.id, email: foundUser.email, role: foundUser.role }, process.env.JWT_SECRET || "your-secret-key", { expiresIn: "24h" });
      ...
      res.json({
        ...
        user: {
          ...
          pincode: pincode || foundUser.pincode,
          role: foundUser.role
        }
      });
    } catch (error) {
